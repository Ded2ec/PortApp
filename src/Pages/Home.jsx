...
const CTAButton = memo(({ href,text, icon: Icon ,handleChangeValue}) => (
  
  <a href={href} >
    <button onClick={handleChangeValue} className="group relative w-[160px]" >
...
const CCTAButton = memo(({  text, icon: Icon }) => (
...
const Home = () => {
  ...
  const [isHovering, setIsHovering] = useState(false)
   ...
   // ใช้ setValue จาก Context
   const { value, setValue } = useValue();  // ดึงค่า value และ setValue จาก Context
   const [localValue, setLocalValue] = useState(value); // ใช้ค่า value ใน Context เป็นค่าเริ่มต้น
 
   const handleChangeValue2 = () => {
    console.log("value changed to 2");
    setValue(2);
      // เปลี่ยนค่า value เป็น 2 เมื่อกดปุ่ม
  };

  const handleChangeValue0 = () => {
  ...
  useEffect(() => {
    setLocalValue(value); // อัปเดต localValue เมื่อ value ใน Context เปลี่ยนแปลง
    console.log("value updated to:", value);  // การดีบั๊กดูว่า value ถูกอัปเดตหรือไม่
    const initAOS = () => {
      AOS.init({
        once: true,
        offset: 10,
